...
SELECT
    deptno,
    name,
    DECODE(deptno, 101, DECODE(name, 'Audie Murphy', 'BEST!')) ECT
FROM professor;
...
SELECT
    deptno,
    name,
    DECODE(deptno, 101, (DECODE(name, 'Audie Murphy', 'BEST!', null)), null) ECT,
    DECODE(deptno, 101, (DECODE(name, 'Audie Murphy', 'BEST!')), null) ECT,
    DECODE(deptno, 101, (DECODE(name, 'Audie Murphy', 'BEST!'))) ECT
FROM professor;
...
SELECT
    deptno,
    name,
    DECODE(deptno, 101, DECODE(name, 'Audie Murphy', 'BEST!', 'GOOD!'), 'N/V') "비고"
FROM professor;
...
SELECT
    ...
    DECODE(SUBSTR(jumin, 7, 1), 2, '여자', '남자') "성별"
FROM student
WHERE deptno1 = 101;
